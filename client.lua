...
AddEventHandler('vrp_weazel:permissao',function()
	if not servico then
		local jobInfo = {
			['titulo'] = "Fotografo",
			['descricao'] = "- Tire fotos pela Cidade</br> - Ganhe por Foto</br> ",
			['callBackEvent'] = "vrp_weazel:checkIn"
		}
		checkIn.openJob(jobInfo)
	end
end)

RegisterNetEvent('vrp_weazel:checkIn')
AddEventHandler('vrp_weazel:checkIn',function()
